...
<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜclawxなのか">なぜItem ClawXなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**Item ClawX**は、強力なAIエージェントと日常のユーザーとの間のギャップを埋めます。[OpenClaw](https://github.com/OpenClaw)をベースに構築されており、コマンドラインによるAIオーケストレーションを、アクセスしやすく美しいデスクトップ体験に変換します。ターミナルは不要です。

ワークフローの自動化、AI搭載チャネルの管理、インテリジェントなタスクのスケジューリングなど、Item ClawXはAIエージェントを効果的に活用するために必要なインターフェースを提供します。

Item ClawXはベストプラクティスのモデルプロバイダーが事前設定されており、Windowsおよび多言語設定をネイティブにサポートしています。もちろん、**設定 → 詳細設定 → 開発者モード**から高度な設定を微調整することもできます。
...
AIエージェントの構築にコマンドラインの習得は不要であるべきです。Item ClawXはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**

| 課題 | Item ClawXのソリューション |
|------|----------------------|
...
Item ClawXは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
`@agent` で別のエージェントを選ぶと、Item ClawX はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離は OpenClaw の sandbox 設定に依存します。
...
Item ClawX はドキュメント処理スキル（`pdf`、`xlsx`、`docx`、`pptx`）もフル内容で同梱し、起動時に管理スキルディレクトリ（既定 `~/.openclaw/skills`）へ自動配備し、初回インストール時に既定で有効化します。追加の同梱スキル（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`、`bocha-skill`）も既定で有効化されますが、必要な API キーが未設定の場合は OpenClaw が実行時に設定エラーを表示します。  
...
### ☁️ IAMログイン＆クラウドバックアップ
オプションでエンタープライズIAM認証を有効にすると、エージェント設定、スキル、メモリデータの自動クラウドバックアップが利用可能になります。バックアップは設定可能な時間帯（デフォルト22:00〜04:00）に1日1回自動実行され、IAMトークンを使用してデータを安全に圧縮・アップロードします。設定画面からいつでも手動バックアップやクラウドからの復元も可能です。すべてのIAM/バックアップ機能は環境変数で制御され、デフォルトでは無効です。

IAMとクラウドバックアップの環境変数：
- `IAM_ENABLED`：`true`に設定するとIAM認証を有効化（デフォルト：`false`）
- `IAM_API_URL`：IAMサーバーのエンドポイントURL
- `IAM_TIMEOUT`：ログインリクエストのタイムアウト（ミリ秒、デフォルト：`30000`）
- `BACKUP_API_URL`：クラウドバックアップAPIのエンドポイントURL
- `BACKUP_TIMEOUT`：バックアップのアップロード/ダウンロードタイムアウト（ミリ秒、デフォルト：`60000`）
- `BACKUP_WINDOW_START`：日次バックアップウィンドウの開始時刻（デフォルト：`22:00`）
- `BACKUP_WINDOW_END`：日次バックアップウィンドウの終了時刻（デフォルト：`04:00`）

### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。Item ClawXはあなたの好みに自動的に適応します。

### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に Item ClawX が自動的に起動します。
...
Item ClawXを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：
...
Item ClawXには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、Item ClawXは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- Item ClawXはTelegramが有効な場合、プロキシをOpenClawのTelegramチャネル設定にも同期します。
...
Item ClawXは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：

```┌─────────────────────────────────────────────────────────────────┐
│                        Item ClawX デスクトップアプリ                    │
│                                                                  │
│  ┌────────────────────────────────────────────────────────────┐  │
│  │              Electron メインプロセス                         │  │
...
│   ├── services/            # Provider/Secrets/IAM/バックアップサービス
│   │   ├── backup/          # クラウドバックアップ圧縮・アップロード・復元
│   │   ├── iam/             # IAM認証サービス
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がItem ClawXをより良くするのに役立ちます。
...
Item ClawXは優れたオープンソースプロジェクトの上に構築されています：
...
### Item ClawX パートナープログラム 🚀

Item ClawX パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に Item ClawX を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、Item ClawX チームは技術サポート、カスタマイズ、統合を全面的に提供します。
...
Item ClawXは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
