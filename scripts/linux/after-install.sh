...
# Post-installation script for Item ClawX on Linux
...
# Create symbolic link for Item ClawX app binary
if [ -x /opt/Item ClawX/clawx ]; then
    ln -sf /opt/Item ClawX/clawx /usr/local/bin/clawx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/Item ClawX/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/Item ClawX/chrome-sandbox' || true
else
    chmod 0755 '/opt/Item ClawX/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/Item ClawX/resources/apparmor-profile'
...
echo "Item ClawX has been installed successfully."
